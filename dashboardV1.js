...
// PERFIL USUARIO Y GESTIÓN DE INTERFAZ GLOBAL
function setupUIByRole() {
    ...
    // 2. Rellenar datos en el Menú Desplegable del Perfil
    document.getElementById('menuUserName').innerText = fullName;
    document.getElementById('menuUserEmail').innerText = currentUser.email;
    
    // 3. Referencias a Elementos de Interfaz
    const roleLabel = document.getElementById('roleLabel');
    const fabAdd = document.getElementById('fabAdd');
    const fabImport = document.getElementById('fabImport'); // <--- CAMBIADO AL NUEVO BOTÓN FLOTANTE
    const headerTitle = document.getElementById('headerTitle');
    const menuAnalytics = document.getElementById('menuAnalytics');

    // 4. Mostrar/Ocultar según el Rol
    if (currentProfile.role === 'teacher') {
        // --- MODO PROFESOR ---
        if (roleLabel) roleLabel.innerText = "👨‍🏫 Profesor";
        if (headerTitle) headerTitle.innerText = "Mis Proyectos";
        
        // Mostrar Botones Flotantes (Crear e Importar)
        if (fabAdd) fabAdd.style.display = 'flex';
        if (fabImport) fabImport.style.display = 'flex'; // <--- LO MOSTRAMOS
        
        // Mostrar opción de Analíticas en el menú del avatar
        ...
        if (fabImport) fabImport.style.display = 'none'; // <--- LO OCULTAMOS
        
        // Ocultar opción de Analíticas
...
if (fabImport && importInput) {
    ...
    importInput.addEventListener('change', async (e) => {
        ...
        // Cambiar a icono de reloj de arena mientras carga
        ...
        fabImport.style.background = "#ccc"; // Ponerlo gris temporalmente
        ...
        try {
            ...
            for (let bg of backgrounds) {
                ...
                if (zipFile && bg.url) {
                    ...
                    const { error } = await window.sb.storage.from('otome-assets').upload(newPath, blob);
                    if(!error) {
        ...
        // Restaurar aspecto original del botón
