...
export class Engine {
    constructor() {
        ...
        this.gameState = {
            currentScene: null,
            actionQueue: [],
            path: [],        // Historial de ruta para el profesor
    ...
    async runNextAction() {
        // Si la cola está vacía, se acaba el juego (o la escena)
        if (!this.gameState.actionQueue || this.gameState.actionQueue.length === 0) {
            this.finishGame(); 
            return; 
        }

        // Sacar la primera acción de la cola
        ...
        switch (action.action) {
            ...
            case "modify_variable":
                ...
                break; // No cortamos, sigue leyendo
        }
    ...
    handleChoice(option) {
        // Guardamos la decisión tomada para el arbol
        if (this.gameState.path.length > 0) {
            this.gameState.path[this.gameState.path.length - 1].choice = option.text;
        }

        // Si la opción tiene bloques dentro (acciones anidadas), los metemos los primeros en la cola
    ...
    handleCheck(action) {
        ...
        // Inyectar los bloques de la rama Verdadera o Falsa
    ...
    async finishGame() {
        ...
        // Generar etiquetas de variables para la pantalla final
        let varsHtml = "";
        ...
        // Subir a Supabase - CHECKEAR
        const projectId = new URLSearchParams(window.location.search).get('id');
        if (window.sb && projectId) {
            const { data: { user } } = await window.sb.auth.getUser();
            if (user) {
                // Definir la nota oficial para el profesor: Si hay una variable llamada "puntos", usamos esa. Si no, sumamos todo.
